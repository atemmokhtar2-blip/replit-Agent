import { useListNotifications, useMarkNotificationRead, useMarkAllNotificationsRead, getGetUnreadCountQueryKey, getListNotificationsQueryKey } from "@workspace/api-client-react";
...
export default function Notifications() {
  ...
  const { data: notifications, isLoading } = useListNotifications({ page: 1, per_page: 50 });
  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6 max-w-4xl mx-auto w-full">
      <div className="flex items-center justify-between space-y-2">
        ...
        <Button variant="outline" onClick={handleMarkAllRead} disabled={markAllReadMutation.isPending || !notifications?.items.some(n => !n.is_read)}>
            ...
            <Card key={notification.id} className={!notification.is_read ? "border-primary/50 bg-primary/5" : ""}>
              <CardContent className="p-4 flex items-start gap-4">
                ...
                {!notification.is_read && (
                  <Button variant="ghost" size="icon" onClick={() => handleMarkRead(notification.id)} disabled={markReadMutation.isPending}>

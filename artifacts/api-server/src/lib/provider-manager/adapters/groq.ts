...
export const groqAdapter: ProviderAdapter = {
  ...
  defaultModels: {
    ...
    verification:  "llama-3.1-8b-instant",
  ...
  async complete(messages: LLMMessage[], options: LLMOptions, apiKey: string) {
    ...
    const tid   = setTimeout(() => ctrl.abort(new Error("GroqTimeout")), TIMEOUT_MS);
    ...
    try {
      ...
      if (!content) {
        const pe: ProviderError = { kind: "incomplete_response", message: "Empty Groq response", retryable: true, waitMs: 0, suggestNextProvider: false };
        throw Object.assign(new Error("Empty Groq response"), { providerError: pe });
      }
      return {
        content,
